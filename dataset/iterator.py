...
class DetRecordIter(mx.io.DataIter):
    ...
    def _get_batch(self):
        ...
        # cv2.imwrite('tmp.jpg', np.transpose(self._batch.data[0][0].asnumpy(), (1, 2, 0)))
        # img = cv2.imread('tmp.jpg')
        # first_label = self._batch.label[0][0].asnumpy()
        # x1 = round((float(first_label[7]))*300)
        # y1 = round((float(first_label[8]))*300)
        # x2 = round((float(first_label[9]))*300)
        # y2 = round((float(first_label[10]))*300)
