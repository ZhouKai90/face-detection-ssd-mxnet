...
def load_widerface(annoFiles, imageSet, dirname, shuffle=False):
    """
    wrapper function for loading ms widerface dataset
    ...
    dirname: str
        root dir for widerface
    shuffle: boolean
        initial shuffle
    """
    # annoFiles = ['instances_' + y.strip() + '.json' for y in imageSet.split(',')]
    assert annoFiles, "No image set specified"
    imdbs = []

    # annoFile = os.path.join(dirname, 'annotations', af)
    imdbs.append(Widerface(annoFiles, imageSet, dirname, shuffle = shuffle))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        db = load_widerface(args.annotations_file, args.set, args.root_path, args.shuffle)
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    else:
    ...
    # im2rec_path = os.path.join(os.getcwd(), '..', 'mxnet/im2rec.py')
